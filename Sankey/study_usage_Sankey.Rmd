...
```{r}
study_usage <- read.table("study_update_KH_20Jun2023.txt", sep="\t", header = T,
...
```{r}
rows_use <- nrow(study_usage)-1#because the last line is different data
left_node_names <- unique(study_usage$`Reference Short 2`[1:rows_use])
right_node_names <- c(colnames(study_usage)[5:ncol(study_usage)])
all_node_names <- c(left_node_names, right_node_names)
